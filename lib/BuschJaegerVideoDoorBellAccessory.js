...
function BuschJaegerVideoDoorBellAccessory(platform, Service, Characteristic, actuator, channel = null, mapping = null) {
    ...
    this.platform.api.publishCameraAccessories(name, [videodoorbellAccessory]);
}
...
BuschJaegerVideoDoorBellAccessory.prototype = {
    ...
    updateCharacteristics: function() {
        ...
        this.getOn(function(joker, value) {
            if (value) {
                that.platform.log('Door bell ringing!');
                // Switch turned on
                that.hiddenServices.videoDoorbell.getService(that.Service.Doorbell).getCharacteristic(that.Characteristic.ProgrammableSwitchEvent).setValue(0);
                // Turn it off
