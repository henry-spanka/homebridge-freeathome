...
BuschJaegerThermostatAccessory.prototype = {
    getCurrentHeatingCoolingState: function(callback) {
        if (callback) {
            ...
            let targetTempDiff = this.getValue(this.channel, 'odp0007');
            if (targetTempDiff < 0) {
                mode = this.Characteristic.CurrentHeatingCoolingState.OFF;
            } else {
                let targetMode = this.getValue(this.channel, 'odp0008');
    ...
    getTargetTemperature: function(callback) {
        if (callback) {
            callback(null, Math.round(this.getValue(this.channel, 'odp0006')));
        }
    },
